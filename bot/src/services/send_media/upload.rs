use crate::{
    ...
    services::messenger::{
        MessengerError, MessengerPort, UploadAudioRequest, UploadPhotoRequest, UploadPhotoUrlRequest, UploadVideoRequest,
    },
};
...
pub struct SendPhotoUrlInput<'a> {
    pub chat_id: i64,
    pub reply_to_message_id: Option<i64>,
    pub photo_url: &'a Url,
    pub with_delete: bool,
...
impl<Messenger> Interactor<SendPhotoInput<'_>> for &SendPhoto<Messenger>
where
    Messenger: MessengerPort,
{
    type Output = String;
    ...
    async fn execute(self, input: SendPhotoInput<'_>) -> Result<Self::Output, Self::Err> {
        self.messenger
            .upload_photo(UploadPhotoRequest {
                chat_id: input.chat_id,
                reply_to_message_id: input.reply_to_message_id,
                media_for_upload: input.media_for_upload,
                name: input.name,
...
impl<Messenger> Interactor<SendPhotoUrlInput<'_>> for &SendPhotoUrl<Messenger>
where
    Messenger: MessengerPort,
{
    type Output = String;
    ...
    async fn execute(self, input: SendPhotoUrlInput<'_>) -> Result<Self::Output, Self::Err> {
        self.messenger
            .upload_photo_url(UploadPhotoUrlRequest {
                chat_id: input.chat_id,
                reply_to_message_id: input.reply_to_message_id,
